...
export default function ChatScreen() {
  ...
  const wide = size === 'lg';
  // DONNA-71: Höhen-Breakpoints für vertikales Compact-Layout
  const shortHeight = containerHeight < 600;
  const veryShortHeight = containerHeight < 500;
  ...
  const inputPadding = compact ? '8px 12px' : '10px 14px';
  const msgMaxWidth = compact ? '88%' : '70%';
  const colMaxWidth = wide ? 760 : compact ? '100%' : 640;
          ...
          <div style={{
            ...
            {messages.length === 0 ? (
              <div style={{
                ...
                gap: veryShortHeight ? 6 : shortHeight ? 10 : 14,
                color: T.muted, userSelect: 'none',
              }}>
                <DonnaAvatar size={veryShortHeight ? 32 : shortHeight ? 44 : 56} animate />
                <div style={{
